(function () {
  ...
  function buildScorecardViewModels(data) {
    ...
    return roster.map(function (person) {
      ...
      // Per-month breakdown for the monthly bar chart (commits + PRs + story points + lines)
      var monthlyBreakdown = months.map(function (m) {
        ...
        return {
          ...
          story_points: row && typeof row.story_points === 'number' ? row.story_points : 0,
          lines_added: row && typeof row.lines_added === 'number' ? row.lines_added : 0,
          lines_removed: row && typeof row.lines_removed === 'number' ? row.lines_removed : 0
        };
      });
  ...
  function renderScorecardCard(vm) {
    ...
    // Dual-series mini-chart (e.g. lines added + deleted on a shared y-axis)
    function buildDualMiniChart(label1, color1, values1, label2, color2, values2, months) {
      var maxVal = Math.max.apply(null, values1.concat(values2).concat([1]));
      var niceMax = Math.ceil(maxVal / 10) * 10;
      if (niceMax < 10) niceMax = 10;
      var gridSteps = 4;
      ...
      var chartTitle = document.createElement('div');
      chartTitle.className = 'scorecard-monthly-mini-title';
      chartTitle.innerHTML = '<span style="color:' + color1 + '">●</span> ' + label1 +
        ' <span style="color:' + color2 + ';margin-left:6px">●</span> ' + label2;
      chartWrap.appendChild(chartTitle);
      ...
      months.forEach(function (m, idx) {
        ...
        var v1 = values1[idx];
        var v2 = values2[idx];
        var h1 = niceMax > 0 ? (v1 / niceMax) * 100 : 0;
        var h2 = niceMax > 0 ? (v2 / niceMax) * 100 : 0;
        ...
        if (v1 > 0) {
          ...
          lab1.className = 'scorecard-month-value';
          lab1.textContent = v1 > 999 ? (v1 / 1000).toFixed(1) + 'k' : v1;
          lab1.style.bottom = 'calc(' + h1 + '% + 2px)';
          lab1.style.left = '0';
          lab1.style.width = '50%';
          lab1.style.color = color1;
          lab1.style.fontSize = '7px';
          barsContainer.appendChild(lab1);
        }
        if (v2 > 0) {
          var lab2 = document.createElement('div');
          lab2.className = 'scorecard-month-value';
          lab2.textContent = v2 > 999 ? (v2 / 1000).toFixed(1) + 'k' : v2;
          lab2.style.bottom = 'calc(' + h2 + '% + 2px)';
          lab2.style.left = '50%';
          lab2.style.width = '50%';
          lab2.style.color = color2;
          lab2.style.fontSize = '7px';
          barsContainer.appendChild(lab2);
        }

        barsContainer.appendChild(bar1);
        barsContainer.appendChild(bar2);

        var tipText = monthShortLabel(m) + ': +' + v1 + ' / −' + v2 + ' lines';
        barsContainer.addEventListener('mouseenter', function (evt) { showBarTooltip(evt, tipText); });
        barsContainer.addEventListener('mousemove', function (evt) { showBarTooltip(evt, tipText); });
        barsContainer.addEventListener('mouseleave', hideBarTooltip);
    ...
    var storyPointValues = mb.map(function (r) { return r.story_points; });
    var linesAddedValues = mb.map(function (r) { return r.lines_added; });
    var linesRemovedValues = mb.map(function (r) { return r.lines_removed; });
    ...
    // Second row: Story Points + Code Lines (different scales)
    var chartsRow2 = document.createElement('div');
    chartsRow2.className = 'scorecard-monthly-charts-row';
    chartsRow2.style.marginTop = '12px';

    chartsRow2.appendChild(buildMiniChart('Story Pts', '#fbbf24', storyPointValues, monthLabels));
    chartsRow2.appendChild(buildDualMiniChart('Lines +', '#34d399', linesAddedValues, 'Lines −', '#fb7185', linesRemovedValues, monthLabels));

    monthlyWrap.appendChild(chartsRow);
    monthlyWrap.appendChild(chartsRow2);
